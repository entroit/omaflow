...
shellcheck install link-local update scripts/preflight.sh scripts/check-local.sh
for script in install link-local update scripts/preflight.sh scripts/check-local.sh; do
  bash -n "$script"
done
git diff --check
